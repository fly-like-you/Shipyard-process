...
def out(block, map1, count, flag, df, curr):
    ...
    if no_out:
        ...
        obstruct_block = find_out(test_map.data, block, flag, test_map, num_map, df)
...
def find_out(block_list, block, flag, map, num_map, df):
    ...
    while True:
        ...
        if no_out:
            ...
            for num, i in enumerate(can_out_block):
                ...
                obstruct_block = check.best_obstruct_blocktest(flag)
                x_axis, y_axis = order.order(df)
                ...
                obstruct_block = best_obstruct_block
                print(obstruct_block)
                input()
