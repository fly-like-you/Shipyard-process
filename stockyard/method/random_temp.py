from stockyard.util import check_maze, maze
import random
import pandas as pd
import math
import copy
...
def insert(block, map1, count, area, df):
    minsize = 50
    maxsize = 255
...
def out(block, map1, count, df, curr):
    ...
    no_out = out_check(block, test_map, 1)
    ...
    if no_out:
        ...
        obstruct_block = find_out(test_map.data, block, test_map, num_map)
...
# 나갈수 있는 블록인지만 체크지
def out_check(block, copy_map, point):
    # print("check", block)
    no_out = False
    pos_loc = []
    ...
    entrance_list.extend(s.find_start(copy_map.flag))
...
def find_out(block_list, block, map, num_map):
    ...
    while True:
        ...
        no_out_list = []
        for i in block_list:
            no_out = out_check(i, map1, 1)  # 블록들마다 나올수 있는지 체크
        ...
        if no_out:
            ...
            for num, i in enumerate(can_out_block):
                ...
                obstruct_block = check.best_obstruct_block(map1.flag)
