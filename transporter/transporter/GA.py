...
def evaluation(arr):
    ...
    for i in range(len(arr.works)): # 트랜스포터의 작업 수 만큼 반복
        if i == 0:
            ...
            pick_up_time = max(times[-1][1], arr.works[i].start_time) # 직전 작업과 비교하여 최대값 선정
...
if __name__ == '__main__':
    ...
    for i in result_population:
        if i.works:
            ...
            work += len(i.works)
            time += evaluation(i)
            for j in range(len(i.works)):
                a.append(i.works[j].no)
                a.sort()

    print(a)
    print('최종 결과:', cnt, time, work)
